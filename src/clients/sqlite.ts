...
import { get_config, debug_log } from '../config.js';
import {
  QueryResult,
  ColumnInfo,
  TableInfo,
...
export function validateDatabasePath(path: string): string {
  ...
  debug_log('Validated database path:', { original: path, resolved: resolvedPath });
  return resolvedPath;
}
 ...
 */
export function openDatabase(path: string, create: boolean = true): Database.Database {
 ...
 */
export function describeTable(databasePath: string, tableName: string): ColumnInfo[] {
  return withErrorHandling(() => {
    const result = executeSelectQuery(
      databasePath,
...
export function backupDatabase(
  ...
  return withErrorHandling(() => {
    ...
    if (backupPath) {
      ...
      const baseName = resolvedSourcePath.split('/').pop()?.replace('.db', '') || 'database';
      resolvedBackupPath = resolve(config.SQLITE_BACKUP_PATH, `${baseName}-${timestamp}.db`);
    }
